 ...
 */
	function __pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		$url = array_merge(array('page' => $paging['page'] + ($which == 'Prev' ? $step * -1 : $step)), $url,array('recCount'=>$options['recCount']));
	...
	function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = $start; $i < $params['page']; $i++) {
				$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i,'recCount'=>$options['recCount']), $options))
					. $separator;
			}
			...
			if ($end != $params['page']) {
				$out .= $this->Html->tag($tag, $this->link($i, array('page' => $end,'recCount'=>$options['recCount']), $options));
			}
